...
void
TextBuffer::SetFont(const Font* pFont)
{
    mp_Font = pFont;
    if (mp_Font)
    {
...
void
TextBuffer::SetText(const string& text)
{
    m_Text = text;
...
void
TextBuffer::SetFontSize(const unsigned int& fontSize)
{
    m_FontSize = fontSize;
...
void
TextBuffer::SetColor(const Color& color)
{
    m_Color = color;
...
void
TextBuffer::SetPos(const Vector2f& pos)
{
    m_SfText.setPosition(pos);
}

Vector2f
TextBuffer::GetPos() const
{
    return m_SfText.getPosition();
}

Vector2f
TextBuffer::GetSize() const
{
...
int
TextBuffer::Script_New(lua_State* L)
{
...
int
TextBuffer::Script_Delete(lua_State* L)
{
