...
void
InputSystem::MapKey(const MappedInputID& id, const Keyboard::Key& key)
{
...
void
InputSystem::MapMouseButton(const MappedInputID& id, const Mouse::Button& button)
{
...
Keyboard::Key
InputSystem::GetMappedKey(const MappedInputID& id)
{
...
Mouse::Button
InputSystem::GetMappedMouseButton(const MappedInputID& id)
{
...
void
InputSystem::TriggerKeyPress(const Keyboard::Key& key)
{
...
void
InputSystem::TriggerKeyRelease(const Keyboard::Key& key)
{
    if (m_MappedKeys.contains_key(key))
    {
...
void
InputSystem::TriggerTextInput(const char32_t& input)
{
    Dispatch(Event(EvtTextInput, TextInputEventData(input)));
}

void
InputSystem::TriggerMouseMoveRelative(const float& dx, const float& dy)
{
    m_MouseX += dx;
    m_MouseY += dy;
...
void
InputSystem::TriggerMouseMoveAbsolute(const float& x, const float& y)
{
    float dx = x - m_MouseX, dy = y - m_MouseY;
...
void
InputSystem::TriggerMouseScroll(const float& dx, const float& dy)
{
    Dispatch(Event(EvtMouseScroll, MouseScrollEventData(dx, dy)));
}

void
InputSystem::TriggerMouseButtonPress(const Mouse::Button& mouseButton)
{
    if (m_MappedMouseButtons.contains_key(mouseButton))
    {
        Dispatch(
            Event(EvtMappedInputPress, MappedInputEventData(m_MappedMouseButtons[mouseButton])));
    }
...
void
InputSystem::TriggerMouseButtonRelease(const Mouse::Button& mouseButton)
{
    if (m_MappedMouseButtons.contains_key(mouseButton))
    {
        Dispatch(
            Event(EvtMappedInputRelease, MappedInputEventData(m_MappedMouseButtons[mouseButton])));
    }
...
void
InputSystem::TriggerMappedInputPress(const MappedInputID& input)
{
    Dispatch(Event(EvtMappedInputPress, MappedInputEventData(input)));
}

void
InputSystem::TriggerMappedInputRelease(const MappedInputID& input)
{
    Dispatch(Event(EvtMappedInputRelease, MappedInputEventData(input)));
}

void
InputSystem::ProcessSfEvent(const sf::Event& sfEvent)
{
...
void
InputSystem::Script_RegisterFunctions(void)
{
...
int
InputSystem::Script_Get(lua_State* L)
{
...
int
InputSystem::Script_MapKey(lua_State* L)
{
...
int
InputSystem::Script_MapMouseButton(lua_State* L)
{
    InputSystem* pInputSystem = (InputSystem*)lua_tointeger(L, 1);
...
int
InputSystem::Script_GetMappedKey(lua_State* L)
{
    InputSystem* pInputSystem = (InputSystem*)lua_tointeger(L, 1);
...
int
InputSystem::Script_GetMappedMouseButton(lua_State* L)
{
    InputSystem* pInputSystem = (InputSystem*)lua_tointeger(L, 1);
