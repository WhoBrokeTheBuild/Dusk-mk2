...
Map<string, int> Logging::s_Levels = Map<string, int>();

Map<string, LogForegroundColor> Logging::s_ForegroundColors = Map<string, LogForegroundColor>();

Map<string, LogBackgroundColor> Logging::s_BackgroundColors = Map<string, LogBackgroundColor>();

Map<string, ArrayList<Logger*>> Logging::s_Loggers = Map<string, ArrayList<Logger*>>();
...
bool
Logging::AddLevel(const int& index, const string& level)
{
...
void
Logging::SetLevelForegroundColor(const string& level, const LogForegroundColor& color)
{
    if (!HasLevel(level))
        return;
...
void
Logging::SetLevelBackgroundColor(const string& level, const LogBackgroundColor& color)
{
    if (!HasLevel(level))
        return;
...
void
Logging::Log(const string& level, const string& message, const string& file, const int& line)
{
...
void
Logging::ExtLog(const string& level, const string& format, const string& file, const int line, ...)
{
    if (!IsLevelShown(level))
        return;
...
void
Logging::Format(const char* level, const char* message, const char* file, const int& line)
{
    ...
    snprintf(s_LogBuffer, DUSK_LOGGING_MAX_BUFFER_SIZE, "+%lli.%lli.%lli.%lli <%s:%d> [%s]: %s",
        hrs, mins, secs, ms, file, line, level, message);
}

void
Logging::DispatchLog(const string& level)
{
    if (!HasLevel(level))
        return;
...
bool
Logging::AddConsoleLogger(const string& level)
{
...
bool
Logging::AddFileLogger(const string& level, const string& filename)
{
    if (!s_Loggers.contains_key(level))
        return false;
...
int
Logging::Script_Log(lua_State* L)
{
    lua_Debug ar;
    lua_getstack(L, 1, &ar);
